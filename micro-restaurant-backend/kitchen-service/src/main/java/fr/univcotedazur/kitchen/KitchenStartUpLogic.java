...
public class KitchenStartUpLogic implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        if (recipeRepository.findAll().size() == 0) { // in case of container restart, mongodb will be already populated
            /* Starters */
            recipeRepository.save(new Recipe("pizza_marg",
                    Post.HOT_DISH,
                    List.of("Take a piece of pizza cool"),
                    300));
            recipeRepository.save(new Recipe("pizza_h",
                    Post.HOT_DISH,
                    List.of("Take a piece of pizza cool!"),
                    300));
            recipeRepository.save(new Recipe("pizza_v",
                    Post.HOT_DISH,
                    List.of("Take a piece of pizza cool!"),
                    300));
            ...
            recipeRepository.save(new Recipe("foie gras",
                    Post.HOT_DISH,
                    List.of("Take foie gras", "Cook it!"),
                    200));
            ...
            recipeRepository.save(new Recipe("soft-boiled egg",
                    Post.COLD_DISH,
                    List.of("Take soft-boiled egg", "Take that", "Put them togther", "Shake", "Ok it\'s finished!"),
                    160));
